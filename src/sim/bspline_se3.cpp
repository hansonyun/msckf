...
void BsplineSE3::feed_trajectory(std::vector<Eigen::VectorXd> traj_points) {
  ...
  // Find the average frequency to use as our uniform time steps
  double sum_dt = 0;
  ...
  dt = (dt < 0.05) ? 0.05 : dt;
  ADEBUG << "[B-SPLINE]: control point dt = " << dt << " (original dt of "
         << sum_dt / (traj_points.size() - 1);
  ...
  for (size_t i = 0; i < traj_points.size() - 1; i++) {
    ...
    Eigen::Vector4d q_coeff = traj_points[i].tail(4);
    T_IinG.block(0, 0, 3, 3) = Eigen::Quaterniond(q_coeff).toRotationMatrix().transpose();
  ...
  ADEBUG << "[B-SPLINE]: trajectory start time = " << timestamp_min;
  ADEBUG << "[B-SPLINE]: trajectory end time = " << timestamp_max;
  ...
  while (true) {
    ...
    // ADEBUG("[SIM]: time curr = %.6f | lambda = %.3f | dt = %.3f | dtmeas =
  ...
  ADEBUG << "[B-SPLINE]: start trajectory time of " << timestamp_start;
}
...
bool BsplineSE3::get_pose(double timestamp, Eigen::Matrix3d &R_GtoI, Eigen::Vector3d &p_IinG) {
  ...
  bool success = find_bounding_control_points(timestamp, control_points, t0, pose0, t1, pose1, t2,
                                              pose2, t3, pose3);
  // ADEBUG("[SIM]: time curr = %.6f | dt1 = %.3f | dt2 = %.3f | dt3 = %.3f | dt4 = %.3f | success =
...
bool BsplineSE3::find_bounding_poses(const double timestamp, const AlignedEigenMat4d &poses,
  ...
  auto lower_bound =
      poses.lower_bound(timestamp); // Finds timestamp or next(timestamp) if not available
