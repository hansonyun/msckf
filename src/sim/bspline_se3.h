 ...
 * \f$\mathbb{SE}(3)\f$ manifold. This is based off of the derivations from [Continuous-Time
 * Visual-Inertial Odometry for Event
 * Cameras](https://ieeexplore.ieee.org/abstract/document/8432102/) and [A Spline-Based Trajectory
 * Representation for Sensor Fusion and Rolling Shutter
 * Cameras](https://link.springer.com/article/10.1007/s11263-015-0811-3) with some additional
 * derivations being available in [these
 * notes](http://udel.edu/~pgeneva/downloads/notes/2018_notes_mueffler2017arxiv.pdf). The use of
 * b-splines for \f$\mathbb{SE}(3)\f$ interpolation has the following properties:
 *
 ...
 *
 * The key idea is to convert a set of trajectory points into a continuous-time *uniform cubic
 * cumulative* b-spline. As compared to standard b-spline representations, the cumulative form
 * ensures local continuity which is needed for on-manifold interpolation. We leverage the cubic
 * b-spline to ensure \f$C^2\f$-continuity to ensure that we can calculate accelerations at any
 ...
 * Note that one needs to ensure that they use the SE(3) matrix expodential, logorithm, and hat
 * operation for all above equations. The indexes correspond to the the two poses that are older and
 * two poses that are newer then the current time we want to get (i.e. i-1 and i are less than s,
 * while i+1 and i+2 are both greater than time s). Some additional derivations are available in
 * [these notes](http://udel.edu/~pgeneva/downloads/notes/2018_notes_mueffler2017arxiv.pdf).
 */
class BsplineSE3 {
...
public:
   ...
   *
   * Our control points need to be uniformly spaced over the trajectory, thus given a trajectory we
   * will uniformly sample based on the average spacing between the pose points specified.
   *
   * @param traj_points Trajectory poses that we will convert into control points (timestamp(s),
   * q_GtoI, p_IinG)
   */
   ...
   * @return False if we can't find it
   */
  bool get_velocity(double timestamp, Eigen::Matrix3d &R_GtoI, Eigen::Vector3d &p_IinG,
...
protected:
  ...
  static bool find_bounding_poses(const double timestamp, const AlignedEigenMat4d &poses,
                                  double &t0, Eigen::Matrix4d &pose0, double &t1,
